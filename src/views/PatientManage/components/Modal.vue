<template>
  <div class="createPatientBox">
        ...
        >
          <el-row :gutter="10">
            <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
              <el-form-item label="医生：">
                <span v-if="isCaseInfo">{{caseForm.doctorName}}</span>
            ...
            <el-col :xs="24" :sm="12" :md="12" :lg="12" :xl="12">
              <el-form-item label="护士：">
                <span v-if="isCaseInfo">{{caseForm.assistantName}}</span>
          ...
          <div class="formTitle">
            ...
            <div class="medical-img">
              <div class="medical-row" v-if="isCaseInfo">
                <div class="medical-row-li" v-for="(item, index) in addImgeArr" :key="index">
                  <img :src="reditImg(item.imgUrl)" class="img_item">
                  <p class="imgRemark" :title="item.remark">{{item.reservationDate}} {{item.remark}}</p>
                </div>
              </div>
              <div class="medical-row"  v-if="!isCaseInfo">
                <div class="medical-row-li" v-for="(item, index) in addImgeArr" :key="index">
                  <i class="deleteIcon el-icon-delete" @click="deleteImage(item, index)"></i>
                  <img :src="reditImg(item.imgUrl)" class="img_item">
                  <p class="imgRemark" :title="item.remark">{{item.reservationDate.substring(0,10)}} {{item.remark}}</p>
                </div>
                <div class="add-img" @click="addMedicalImg">
    ...
    <el-dialog title="关联影像资料" :visible.sync="showImageLibrary" center>
      <div class="imageBox-top">
        <span>影像类型&nbsp;</span>
        <el-select v-model="typeCode" placeholder="请选择" @change="getImageLibrary">
      ...
      <div class="imageBox-body">
        <div v-if="ImageLibraryList.length > 0">
          <div class="imageBox-row" v-for="(item, index) in ImageLibraryList" :key="index" v-if="ImageLibraryList.length > 0">
              ...
              <div class="imageBox-list">
                  <li class="childrenBox"
                    ...
                    >
                   <div>
                     <label style="position:absolute"><input type="checkbox" @change="change(val)" :name="val.code"  :value="val" v-model="val.checked"/></label>
                     <img :src="reditImg(val.imgUrl)" class="img_item">
                     <p class="imgRemark">{{val.remark}}</p>
                  </div>
                  </li>
              </div>
          </div>
          <el-button @click="showImageLibrary = false">取消</el-button>
          <el-button type="primary" @click="chooseMedicalImg">保存</el-button>
        </div>
...
export default {
  ...
  methods: {
      ...
      if ( keyName ==="createCase") {
          this.$refs['form'].validate((valid) => {
            if (valid) {
              ...
              caseForm.remark2 = this.addImgeArr.map(item => item.code).join(',')
              // console.log(caseForm.remark2);
    ...
    showTeethView (num) {
      ...
      if ( num === 1) {
        ...
        if (this.sideArr1.length) {
          this.sideArr1.map(item => {
        ...
        if (this.sideArr4.length) {
          this.sideArr4.map(item => {
            this.reverseShowSide(item)
          })
        }
      } else {
    ...
    chooseTemplate () {
      ...
      this.caseForm = {...this.caseForm, ...this.form}
      this.caseForm.code = ''
    },
    ...
    addMedicalImg () {
      // checked回显
      this.ImageLibraryList.forEach(e => {
        e.list.map(item => {
          this.addImgeCode.map(op =>{
            if (item.code == op) {
              item.checked = true
            }
          })
    ...
    chooseMedicalImg () {
      ...
      this.addImgeArr = arr1
      console.log(this.addImgeArr)
      if (this.addImgeArr && this.addImgeArr.length > 10) {
        this.$message.error('最多选择10张图片')
        return false
      }
    ...
    deleteImage (item, index) {
      this.addImgeArr.splice(index, 1)
    },
  ...
  mounted() {
      ...
      this.getImageType()
      if (this.caseForm.imgs) {
            let aa = this.caseForm.imgs
            aa.forEach(item => {
              item.checked = true
            })
            this.addImgeArr = aa
            this.addImgeCode = this.caseForm.remark2.split(',')
